...
<form id="fm">
    ...
    手机号：<input type="text" id="phone" name="phone" onblur="checkPhone()"><br>
    ...
    街道小区详细地址：<input type="text" name="address"><br>
    提交：<input type="button" value="提交" onclick="add()">
</form>
</body>
<script>
    ...
    function add(){
        $.post(
            "<%=request.getContextPath()%>/receiveAddress/addAddress",
            $("#fm").serialize(),
            function (result){
                if (result.code == 200){
                    parent.location.href = "<%=request.getContextPath()%>/shop/toSettle/" + userId;
                }
                layer.msg(result.msg);
            }
        );
